...
- [Hiring System](issues/07-hiring-system.md) — Implemented candidate hiring transferring applicants to active staff, morale recovery (+2/day idle) and work overload morale decay (-3/day critical bug, -1/day high severity) simulation, transition notifications below 30 morale, morale boosting terminal commands (/raise, /bonus), and success morale boosts.
- [Kanban Board](issues/08-kanban-board.md) — Implemented interactive /backlog kanban columns and developer assignment select box syncing developer currentTicketId and ticket assignedTo, automatically updating statuses from todo to in_progress (and vice-versa), rendering visual progress meters, and preserving assignment history on done.
